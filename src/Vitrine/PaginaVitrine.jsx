...
const Vitrine = () => {
  ...
  const [isDragging, setIsDragging] = useState(false); // Adicionado para rastrear arrasto
    ...
    <div className="container vitrine-page">
      <div className="carousel">
        ...
        <motion.div
          ...
          onDragStart={() => setIsDragging(true)} // Define que está arrastando
          onDragEnd={() => setIsDragging(false)} // Define que parou de arrastar
        >
          {produtosCarregados.map((produto, index) => (
            <motion.div className="item" key={index}>
              <Link
                to={`/paginaProduto/${produto.id}`}
                onClick={(e) => {
                  if (isDragging) e.preventDefault(); // Impede o clique se estiver arrastando
                }}
              >
                <img
